...
void LinkedList<ItemType>::bubbleSort(Comparator compare, int* nodeSwapCounter)
{
    ...
    for (int i=0; i< size_; i++)
    {
        ...
        {
            if (!compare(*(current->getItem()), *(current->getNext()->getItem())))
            {
                ItemType x = current->getItem();
                current->setItem(current->getNext()->getItem());
                current->getNext()->setItem(x);
                (*nodeSwapCounter)++;
            }
            current = current->getNext();
        }
